...
#pragma unroll
                        for (int j = 0; j < 2 * 4; j++) {
                            reinterpret_cast<scalar_t *>(&c)[j] = Dtype::float2num(reinterpret_cast<float *>(
                                &frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)]);
                        }
...
#define __CALL_IF(NUM_BITS, THREAD_M_BLOCKS, THREAD_N_BLOCKS,                                                                                                                                                                                          \
    ...
    else if (num_bits == NUM_BITS && thread_m_blocks == THREAD_M_BLOCKS && thread_n_blocks == THREAD_N_BLOCKS && thread_k_blocks == THREAD_K_BLOCKS && has_act_order == HAS_ACT_ORDER && group_blocks == GROUP_BLOCKS && num_threads == NUM_THREADS) { \
...
bool is_valid_cache_size(thread_config_t const &th_config, int max_m_blocks,
    ...
    float pipe_size = float((a_size + b_size) * pipe_stages);
...
void marlin_mm_f16i4(const void *A, const void *B, void *C, void *s,
    ...
    if (thread_k != -1 && thread_n != -1) {
        ...
        // Auto config
        exec_cfg = determine_thread_config(prob_m, prob_n, prob_k, num_bits, group_size,
                                           has_act_order, is_k_full, max_shared_mem);
    }

    // TORCH_CHECK(exec_cfg.max_m_blocks > 0 && is_valid_config(exec_cfg.tb_cfg, exec_cfg.max_m_blocks, prob_m, prob_n, prob_k, num_bits, group_size, has_act_order, is_k_full, max_shared_mem),
    ...
    for (int i = 0; i < tot_m_blocks; i += exec_cfg.max_m_blocks) {
        ...
        // #define undefined_error TORCH_CHECK(false, "Unsupported shapes: MNK = [" + str(prob_m) + ", " + str(prob_n) + ", " + str(prob_k) + "]" + ", has_act_order = " + str(has_act_order) + ", num_groups = " + str(num_groups) + ", group_size = " + str(group_size) + ", thread_m_blocks = " + str(thread_m_blocks) + ", thread_n_blocks = " + str(thread_n_blocks) + ", thread_k_blocks = " + str(thread_k_blocks));
...
void gptq_marlin_mm_fp16(void *c, const void *a, const void *b, const void *scale,
    ...
    marlin_mm_f16i4<half>(a, b, c, (void *)scale, nullptr, nullptr,
                          workspace, m, n, k, (char *)workspace + m * k * 2,
                          num_bits, false, true, num_groups, group_size, device_id, stream, -1, -1, -1, int(max_par));
}
...
void gptq_marlin_mm_bf16(void *c, const void *a, const void *b, const void *scale,
                         ...
                         int num_groups, int group_size,
                         int device_id, cudaStream_t stream) {
    marlin_mm_f16i4<nv_bfloat16>(a, b, c, (void *)scale, nullptr, nullptr,
                                 workspace, m, n, k, (char *)workspace + m * k * 2,
                                 num_bits, false, true, num_groups, group_size, device_id, stream, -1, -1, -1, int(max_par));
}
} // namespace gptq_marlin
